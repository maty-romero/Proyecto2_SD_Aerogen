...
export const useWindFarmData = (options: UseWindFarmDataOptions = {}) => {
   ...
   */
  const handleTurbineUpdate = useCallback((turbineId: string, message: MqttTurbineMessage) => {
    setData(prev => {
      ...
      const updatedTurbine: Turbine = {
        id: turbineId,
        name: existingTurbine?.name || `Turbina ${turbineId}`,
        status: message.status,
        capacity: existingTurbine?.capacity || 2.5,
