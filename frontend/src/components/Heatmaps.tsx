...
export function Heatmaps({ turbines }: HeatmapsProps) {
  ...
  // 4. Heatmap de Temperatura de Caja de Cambios
  const maxTemp = Math.max(...turbines.map(t => t.mechanical.gearboxTemperature));
  const minTemp = Math.min(...turbines.map(t => t.mechanical.gearboxTemperature));
  const tempData = turbines.map(t => {
    const normalized = maxTemp > minTemp 
      ? (t.mechanical.gearboxTemperature - minTemp) / (maxTemp - minTemp)
      : 0;
    return {
      id: t.id,
      name: t.name,
      value: t.mechanical.gearboxTemperature,
      normalized,
      display: `${t.mechanical.gearboxTemperature.toFixed(1)}°C`,
    };
  });

  // 5. Heatmap de Vibraciones
  const maxVibration = Math.max(...turbines.map(t => t.mechanical.vibration));
  const vibrationData = turbines.map(t => {
    const normalized = maxVibration > 0 ? t.mechanical.vibration / maxVibration : 0;
    return {
      id: t.id,
      name: t.name,
      value: t.mechanical.vibration,
      normalized,
      display: `${t.mechanical.vibration.toFixed(2)} mm/s`,
    };
  });

  // 6. Heatmap de Disponibilidad (simulado basado en status)
  ...
  // 7. Heatmap de Velocidad de Viento
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="power" className="w-full">
        ...
        <TabsContent value="mechanical" className="space-y-6 mt-6">
          {renderHeatmap(
            tempData, 
            ...
            true,
            {
              minLabel: minTemp.toFixed(0),
              maxLabel: maxTemp.toFixed(0),
              unit: '°C'
            }
          )}
          {renderHeatmap(
            ...
            {
              minLabel: '0',
              maxLabel: maxVibration.toFixed(2),
              unit: 'mm/s'
            }
          )}
        </TabsContent>
